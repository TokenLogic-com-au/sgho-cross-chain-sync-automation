...
5. On-chain, **`ReceiverTemplate.onReport`** validates the Keystone forwarder and the pinned workflow **author** (plus id/name if configured), then **`SyncKeeperConsumer._processReport`** re-reads both balances and re-derives the surplus token. Any unactionable state **reverts** (the CRE workflow does not retry a reverted report, and `needsUpkeep` already gated submission on the same checks): `OraclePoolNotSet`, `SyncNotNeeded`, `SyncInCooldown`, or `StalePrice`. Otherwise it prices `minAmountOut` from the sGHO/GHO feed and calls **`SwapHandler.sync`** with the surplus token and the on-chain `feeData` (native fee when not paid in `GHO`), emitting `SyncPerformed`.
...
| `writeReport` reverts         | Receiver not `IReceiver` / wrong forwarder / **report author ≠ pinned `expectedAuthor`** / insufficient gas / consumer not granted `SYNC_ROLE` / on-chain `feeData` encoding vs `SwapHandler` mismatch. Unactionable states also revert (`OraclePoolNotSet`, `SyncNotNeeded`, `SyncInCooldown`, `StalePrice`) — these should be rare since `needsUpkeep` gates submission on the same checks, but a pool that moves between the gate read and execution will revert here rather than sync. |
